	...
	if(!empty($marker_users_shadow_url)) {
        ...
        if($place_id == 'all') {
            if($post_type=='users'){
                ...
                $post_array = apply_filters('gpress_post_array',$post_array);
                $default_query_args['post_type'] = $post_array;
                if(empty($max_places)) {
                    $query_args['posts_per_page'] = -1;
                }else{
                    $query_args['posts_per_page'] = $max_places;
                }
                ...
                if ( have_posts() ) : while ( have_posts() ) : the_post();
                    ...
                    //echo '$this_post_type = '.$this_post_type; exit;
                    $meta = get_post_meta($post->ID,'_gpress_posts',TRUE);
                    $meta_latlng = $meta['latlng'];
                    if(empty($meta_latlng)){
                        $geo_lat = get_post_meta($post->ID,'geo_latitude',TRUE);
                        $geo_lng = get_post_meta($post->ID,'geo_longitude',TRUE);
                        if((!empty($geo_lat))&&(!empty($geo_lng))){
                            $meta_latlng = $geo_lat.','.$geo_lng;
                        }
                    }
                    //echo '$meta_latlng = '.$meta_latlng; exit;
                    if($meta_latlng){
                        $this_post_type = $post->post_type;
                        $place_array[$post->ID]['latlng'] = $meta_latlng;
